<template>
 <div class="article">
     ...
     <div :v-if="isAuthor || category" class="bottom">
         ...
         <button v-if="isAuthor" class="mini negative ui button"  type="submit" @click.prepend="delete">削除</button>
     </div>
 </div>
</template>
...
export default {
  ...
  methods: {
      delete() {
          console.log("delete");
        //   this.deleteArticles({userId: this.authorId, timestamp: this.timestamp})
      }
  }
};
</script>
