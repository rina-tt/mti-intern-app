<template>
  ...
  <div>
    ...
    <div v-if="!isLoading" class="container">
      <!-- 基本的なコンテンツはここに記載する -->
      <PostArticle :getArticles=getArticles /> 
  ...
  methods: {
    ...
    async getArticles({userId, category, startDate, endDate}) {
      ...
      try {
            /* global fetch */
        this.isLoading = true;
        const  res = await fetch(baseUrl + `/articles?userId=${userId === undefined ? "" : userId}&category=${category}&startDate=${startDate}&endDate=${endDate}`,  {
            method: 'GET',
            headers
        });
